...
export default () => {
    ...
    const [imageURL, setImageURL] = useState<string>("");
    const state = useSelector((state: RootState) => state.invitation.Invitation);
    const [myTemplate, setMyTemplate] = useState<Template>({} as Template);
    const navigate = useNavigate();

    // --- Fetch Template and Image URL ---
    useEffect(() => {
        const storedTemplate = JSON.parse(sessionStorage.getItem("template") || '{}');
        setMyTemplate(storedTemplate);
    ...
    // --- Load Background Image and Add Text to Canvas ---
    useEffect(() => {
        if (!canvas || !imageURL) return;
    ...
    const addTextToCanvas = (scaleFactor: number) => {
        ...
        const invitationTexts = Object.entries(state.text);

        myTemplate.templateFields.forEach((field) => {
            const matchingEntry = invitationTexts.find(([key]) => key === field.field.name);
            const matchingText = matchingEntry ? matchingEntry[1] : undefined;
            ...
            if (matchingText) {
                ...
                (textObj as any).fieldName = field.field.name; // Tagging the text object with its field name
                canvas.add(textObj);
            }
        });
        canvas.renderAll();
    ...
    const handleSaveAndDownload = async () => {
        ...
        try {
            ...
            const fileUrl = response.data; // This is the URL to be stored in the DB

            // Prepare invitation data for saving
            const invitationData = {
                category: JSON.parse(sessionStorage.getItem("category") || '')?.id,
                name: newFile.name,
                imageUrl: fileUrl, // Use the presigned URL here
                userId: decodeToken()?.decoded.userId,
                templateId: myTemplate.id,
                content: "הזמנה מושלמת", // Consider making this more dynamic if needed
            };

            // Save completed invitation details to the backend
            await axios.post(`${import.meta.env.VITE_API_URL}/CompletedInvitation/add`, invitationData);
    ...
    const handlePrint = () => {
        ...
        canvas.renderAll(); // Ensure canvas is fully rendered before converting to image
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            '&:hover': {
                                ...
                                borderColor: 'darken(var(--primary-color), 10%)',
                            },
                            fontSize: { xs: '14px', md: '18px' },
                            padding: { xs: '8px 12px', md: '10px 15px' },
                            gap: '8px',
