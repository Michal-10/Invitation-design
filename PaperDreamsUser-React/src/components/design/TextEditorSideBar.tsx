...
// src/components/design/TextEditorSidebar.tsx
...
import {
  Box, Button, FormControl, IconButton, InputLabel, MenuItem,
  Paper, Select, TextField, Typography, Card, CardContent
} from "@mui/material"
import {
  FormatBold, ArrowUpward, ArrowBack, ArrowForward, ArrowDownward,
  Close, Edit, TextFields
} from "@mui/icons-material"
...
export default function TextEditorSidebar({
  ...
  return (
    <Paper elevation={3} sx={{ borderRadius: 2, direction: 'ltr', height: "100%", display: "flex", flexDirection: "column" }}>
      <Box sx={{ p: 2, bgcolor: "var(--primary-color)", color: "white", textAlign: "center" }}>
      ...
      <Box sx={{ p: 2, flexGrow: 1, overflowY: "auto" }}>
        {!isEditing ? (
          <>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
              {fieldsWithPlaces.map((field, idx) => {
                const obj = canvas.getObjects().find((o) => (o as any).fieldName === field.field.name) as fabric.Text
                const label = obj?.text ? `לעריכת ${field.field.name}` : field.field.name

                return (
                  <Button key={idx} onClick={() => handleFieldSelect(field)} variant="outlined" startIcon={<TextFields />} sx={{ textAlign: "start" }}>
                    {label}
                  </Button>
                )
              })}
          ...
          <Card sx={{ position: "relative", boxShadow: "none" }}>
            <CardContent>
              ...
              <FormControl fullWidth sx={{ mb: 2 }}>
                <InputLabel>גופן</InputLabel>
                <Select value={fontFamily} onChange={handleFontFamilyChange} label="גופן">
                  <MenuItem value="Arial">Arial</MenuItem>
                  <MenuItem value="Times New Roman">Times New Roman</MenuItem>
                  <MenuItem value="Courier New">Courier New</MenuItem>
                  <MenuItem value="Verdana">Verdana</MenuItem>
              ...
              <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                <Box>
                  <Typography variant="body2" fontWeight={600}>צבע טקסט</Typography>
                  <input type="color" value={textColor} onChange={handleColorChange} style={{ width: "100%" }} />
                </Box>
                <Box>
                  <Typography variant="body2" fontWeight={600}>צבע רקע</Typography>
                  <input type="color" value={bgColor === "transparent" ? "#ffffff" : bgColor} onChange={handleBgColorChange} style={{ width: "100%" }} />
                </Box>
              </Box>
              ...
              <Typography variant="subtitle2" sx={{ mt: 3, mb: 1, textAlign: "center" }}>
                הזזת טקסט
              </Typography>

              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                <IconButton onClick={() => moveText(0, -5)}><ArrowUpward /></IconButton>
                <Box sx={{ display: "flex", gap: 2 }}>
                  <IconButton onClick={() => moveText(-5, 0)}><ArrowBack /></IconButton>
                  <IconButton onClick={() => moveText(5, 0)}><ArrowForward /></IconButton>
                </Box>
                <IconButton onClick={() => moveText(0, 5)}><ArrowDownward /></IconButton>
              </Box>
            </CardContent>
