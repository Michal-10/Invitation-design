...
export default function TextEditorSidebar({ canvas, fieldsWithPlaces }: { canvas: fabric.Canvas; fieldsWithPlaces: TemplateField[] }) {
  ...
  const handleFieldSelect = (templateField: TemplateField) => {
    ...
    if (canvas) {
      const textObj = canvas.getObjects().find((obj) => obj.left === templateField.x && obj.top === templateField.y) as fabric.Text
